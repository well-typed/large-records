...
genIndexedOverwrite opts@Options{..} r@Record{..} = do
  ...
  where
    ...
    body
      | allFieldsStrict =
          [| \n t !val -> $(recordFromForcedVectorQ opts r) (
...
genRecordVal opts r@Record{..} mkFn = do
    ...
    mkFn (map varP vars) [|
        $(recordFromUnforcedVectorQ opts r) $(mkVector qUnsafeCoerce vars)
      |]
  where
...
-- > (..) . repToVector
--
-- where the @(..)@ is generated by 'recordFromUnforcedVectorQ' (which will
-- force the vector if needed).
genTo :: Options -> Record -> Q Exp
genTo opts r = [| $(recordFromUnforcedVectorQ opts r) . repToVector |]
...
-- | Construct record from a vector
--
-- If the elements in the vector are already forced, nothing to do; see also
-- 'recordFromUnforcedVectorQ'.
recordFromForcedVectorQ :: Options -> Record -> Q Exp
recordFromForcedVectorQ opts = conE . nameRecordInternalConstr opts

-- | Construct record from a vector where some elements might not be forced.
--
-- If some elements might not be forced, depending on options we might have to
-- force them here.
recordFromUnforcedVectorQ :: Options -> Record -> Q Exp
recordFromUnforcedVectorQ opts@Options{..} r
    | allFieldsStrict = [|
          (\v -> rnfVectorAny v `seq` $(recordFromForcedVectorQ opts r) v)
        |]
    | otherwise =
        recordFromForcedVectorQ opts r
